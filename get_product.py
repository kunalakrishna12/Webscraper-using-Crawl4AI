...
import random
from products_information import main_cat
...
async def filter_category_links(search_links):
    ...
    return [link for link in search_links if "proddetail" in link]
...
if __name__ == "__main__":
    ...
    search_links = asyncio.run(get_google_search_links("CHIRALCEL OJ-H", num_pages=10))
    print(f":mag: Found {len(search_links)} search links.")
    product_links = asyncio.run(filter_category_links(search_links))
    print(f":mag: Found {len(product_links)} Product Links.")
    asyncio.run(main_cat(product_links))
    time.sleep(5)
    print("Done")